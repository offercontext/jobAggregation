        ...
        evidence.append(
            {
                ...
                "meta": " · ".join(
                    value
                    for value in (application.position_name, application.status)
                    if value
                ),
                "source": "pending_action",
            }
...
def _build_add_note_undo(
    ...
    try:
        ...
        return {}
    fields = (
        "application_id", "company", "position", "round", "date", "questions",
        "self_reflection", "difficulty_points", "mood",
    )
    return {
        "kind": "delete_note",
        "label": "撤销保存复盘",
...
def note_specs() -> tuple[ToolSpec[Any, Any], ...]:
    note_fields = tuple(
        ...
        for field, value_type in (
            ...
            ("date", "datetime"), ("allow_placeholder_date", "boolean"),
            ("questions", "long_text"), ("self_reflection", "long_text"),
            ("difficulty_points", "long_text"), ("mood", "long_text"),
        )
    )
    ...
    update_parent = _resolver(
        ...
        resolver_id="note_application_parent", arg_path="id",
        presence="required", resolve=_note_binding,
    )
    ...
    return (
        build_tool_spec(
            contract=provider_contract("list_notes", "List interview review notes. Optionally filter by application id.", {"type": "object", "properties": {"application_id": {"type": "integer"}}}),
            domains=(ToolDomain.NOTES,), dependencies=(),
        ...
        build_tool_spec(
            contract=provider_contract("add_note", "Add an interview review note. If application_id is present, company and position can be omitted.", _schema([])),
            domains=(ToolDomain.NOTES,), dependencies=(),
            required_capability=ToolCapability.NOTES_WRITE,
            binding_contract=BindingContract("optional_target", "application"),
            resolver_bindings=(add_resolver,), confirmation_policy="required",
            editable_fields=note_fields, operation=add_operation,
            ...
            presentation=ToolPresentationBindingV1(
                ...
                success_summary_projector=compact_json,
            ), decoder=_decode, executor=_add, preflight=_validate_add,
        ...
        build_tool_spec(
            contract=provider_contract("update_note", "Update an existing interview review note. Missing fields keep existing values.", _schema(["id"])),
            domains=(ToolDomain.NOTES,), dependencies=("list_notes",),
            required_capability=ToolCapability.NOTES_WRITE,
            binding_contract=BindingContract("enforce_if_bound", "application"),
            resolver_bindings=(update_parent, update_application),
            confirmation_policy="required", editable_fields=note_fields,
        ...
        build_tool_spec(
            contract=provider_contract("delete_note", "Delete an interview review note by id.", {"type": "object", "properties": {"id": {"type": "integer", "description": "Note id."}}, "required": ["id"]}),
            domains=(ToolDomain.NOTES,), dependencies=("list_notes",),
            required_capability=ToolCapability.NOTES_WRITE,
            binding_contract=BindingContract("enforce_if_bound", "application"),
            ...
            presentation=ToolPresentationBindingV1(
                ...
                pending_details_projector=_empty_pending_details,
                success_summary_projector=compact_json,
            ), decoder=_decode, executor=_delete, success_renderer=compact_json,
        ),
    )
